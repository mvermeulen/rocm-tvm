...
do
	...
	tvmc -v run -device "cuda" --profile --file-mode random --print-time --repeat 100 $compiledb_gpu_untuned > $profiledb_gpu_untuned
    fi
    if [ ! -f $profiledb_gpu_autotvm ]; then
	echo "Profiling $profiledb_gpu_autotvm --" | tee -a $logfile
	tvmc -v run -device "cuda" --profile --file-mode random --print-time --repeat 100 $compiledb_gpu_autotvm > $profiledb_gpu_autotvm
    fi
    if [ ! -f $profiledb_gpu_autoscheduler ]; then
	echo "Profiling $profiledb_gpu_autoscheduler --" | tee -a $logfile
	tvmc -v run -device "cuda" --profile --file-mode random --print-time --repeat 100 $compiledb_gpu_autoscheduler > $profiledb_gpu_autoscheduler
    fi
	...
	tvmc -v run -device "cuda" --profile --file-mode random --print-time --repeat 100 $compiledb_gpu_untuned > $rundb_gpu_untuned
    fi
    if [ ! -f $rundb_gpu_autotvm ]; then
	echo "Profiling $rundb_gpu_autotvm --" | tee -a $logfile
	tvmc -v run -device "cuda" --profile --file-mode random --print-time --repeat 100 $compiledb_gpu_autotvm > $rundb_gpu_autotvm
    fi
    if [ ! -f $rundb_gpu_autoscheduler ]; then
	echo "Profiling $rundb_gpu_autoscheduler --" | tee -a $logfile
	tvmc -v run -device "cuda" --profile --file-mode random --print-time --repeat 100 $compiledb_gpu_autoscheduler > $rundb_gpu_autoscheduler
    fi
	...
	tvmc -v run -device "cuda" --profile --file-mode random --print-time --repeat 100 $compiledb_cpu_untuned > $rundb_cpu_untuned
    fi
    if [ ! -f $rundb_cpu_autotvm ]; then
	echo "Profiling $rundb_cpu_autotvm --" | tee -a $logfile
	tvmc -v run -device "cuda" --profile --file-mode random --print-time --repeat 100 $compiledb_cpu_autotvm > $rundb_cpu_autotvm
    fi
    if [ ! -f $rundb_cpu_autoscheduler ]; then
	echo "Profiling $rundb_cpu_autoscheduler --" | tee -a $logfile
	tvmc -v run -device "cuda" --profile --file-mode random --print-time --repeat 100 $compiledb_cpu_autoscheduler > $rundb_cpu_autoscheduler
    fi    
