...
do
	...
	tvmc -v tune --target "cuda" --output $tunedb_gpu_autotvm $onnxfile 2>&1 | tee -a $logfile	
    fi
    if [ ! -f $tunedb_gpu_autoscheduler ]; then
	echo "Tuning $tunedb_gpu_autoscheduler ---" | tee -a $logfile
	tvmc -v tune --target "cuda" --output $tunedb_gpu_autoscheduler --enable-autoscheduler $onnxfile 2>&1 | tee -a $logfile		
    fi    
	...
	tvmc -v compile --target "cuda" --output $compiledb_gpu_untuned $onnxfile 2>&1 | tee -a $logfile
    fi
    if [ ! -f $compiledb_gpu_autotvm ]; then
	echo "Compiling $compiledb_gpu_autotvm ---" | tee -a $logfile	
	tvmc -v compile --target "cuda" --output $compiledb_gpu_autotvm --tuning-records $tunedb_gpu_autotvm $onnxfile 2>&1 | tee -a $logfile
    fi
    if [ ! -f $compiledb_gpu_autoscheduler ]; then
	echo "Compiling $compiledb_gpu_autoscheduler ---" | tee -a $logfile	
	tvmc -v compile --target "cuda" --output $compiledb_gpu_autoscheduler --tuning-records $tunedb_gpu_autoscheduler $onnxfile 2>&1 | tee -a $logfile
    fi
	...
	tvmc -v run --device "cuda" --profile --fill-mode random --print-time --repeat 100 $compiledb_gpu_untuned > $profiledb_gpu_untuned
    fi
    if [ ! -f $profiledb_gpu_autotvm ]; then
	echo "Profiling $profiledb_gpu_autotvm --" | tee -a $logfile
	tvmc -v run --device "cuda" --profile --fill-mode random --print-time --repeat 100 $compiledb_gpu_autotvm > $profiledb_gpu_autotvm
    fi
    if [ ! -f $profiledb_gpu_autoscheduler ]; then
	echo "Profiling $profiledb_gpu_autoscheduler --" | tee -a $logfile
	tvmc -v run --device "cuda" --profile --fill-mode random --print-time --repeat 100 $compiledb_gpu_autoscheduler > $profiledb_gpu_autoscheduler
    fi
	...
	tvmc -v run --device "cpu" --profile --fill-mode random --print-time --repeat 100 $compiledb_cpu_untuned > $profiledb_cpu_untuned
    fi
    if [ ! -f $profiledb_cpu_autotvm ]; then
	echo "Profiling $profiledb_cpu_autotvm --" | tee -a $logfile
	tvmc -v run --device "cpu" --profile --fill-mode random --print-time --repeat 100 $compiledb_cpu_autotvm > $profiledb_cpu_autotvm
    fi
    if [ ! -f $profiledb_cpu_autoscheduler ]; then
	echo "Profiling $profiledb_cpu_autoscheduler --" | tee -a $logfile
	tvmc -v run --device "cpu" --profile --fill-mode random --print-time --repeat 100 $compiledb_cpu_autoscheduler > $profiledb_cpu_autoscheduler
    fi
	...
	tvmc -v run --device "cuda" --fill-mode random --print-time --repeat 100 $compiledb_gpu_untuned > $rundb_gpu_untuned
    fi
    if [ ! -f $rundb_gpu_autotvm ]; then
	echo "Profiling $rundb_gpu_autotvm --" | tee -a $logfile
	tvmc -v run --device "cuda" --fill-mode random --print-time --repeat 100 $compiledb_gpu_autotvm > $rundb_gpu_autotvm
    fi
    if [ ! -f $rundb_gpu_autoscheduler ]; then
	echo "Profiling $rundb_gpu_autoscheduler --" | tee -a $logfile
	tvmc -v run --device "cuda" --fill-mode random --print-time --repeat 100 $compiledb_gpu_autoscheduler > $rundb_gpu_autoscheduler
    fi
	...
	tvmc -v run --device "cpu" --fill-mode random --print-time --repeat 100 $compiledb_cpu_untuned > $rundb_cpu_untuned
    fi
    if [ ! -f $rundb_cpu_autotvm ]; then
	echo "Profiling $rundb_cpu_autotvm --" | tee -a $logfile
	tvmc -v run --device "cpu" --fill-mode random --print-time --repeat 100 $compiledb_cpu_autotvm > $rundb_cpu_autotvm
    fi
    if [ ! -f $rundb_cpu_autoscheduler ]; then
	echo "Profiling $rundb_cpu_autoscheduler --" | tee -a $logfile
	tvmc -v run --device "cpu" --fill-mode random --print-time --repeat 100 $compiledb_cpu_autoscheduler > $rundb_cpu_autoscheduler
    fi    
