...
do
	...
	tvmc -v run --device "rocm" --fill-mode random --print-time --repeat 100 $compiledb_gpu_untuned > $rundb_gpu_untuned
    fi
    if [ ! -f $rundb_gpu_autotvm ]; then
	echo "Profiling $rundb_gpu_autotvm --" | tee -a $logfile
	tvmc -v run --device "rocm" --fill-mode random --print-time --repeat 100 $compiledb_gpu_autotvm > $rundb_gpu_autotvm
    fi
    if [ ! -f $rundb_gpu_autoscheduler ]; then
	echo "Profiling $rundb_gpu_autoscheduler --" | tee -a $logfile
	tvmc -v run --device "rocm" --fill-mode random --print-time --repeat 100 $compiledb_gpu_autoscheduler > $rundb_gpu_autoscheduler
    fi
